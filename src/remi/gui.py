...
class Widget(Tag):
    ...
    def set_on_mouse_hover_listener(self, callback, *userdata):
        """Registers the listener for the Widget.onfocus event.
        ...
        Args:
            callback (function): Callback function pointer.
        """
        self.attributes[self.EVENT_ONHOVER] = \
            "sendCallback('%s','%s');" \
            "event.stopPropagation();event.preventDefault();" \
            "return false;" % (self.identifier, self.EVENT_ONHOVER)
        self.eventManager.register_listener(self.EVENT_ONHOVER, callback, *userdata)
