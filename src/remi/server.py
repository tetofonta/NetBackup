...
def getconnstring():
    ...
    """
    global usehttps
    if usehttps: return "ws = new WebSocket('ws://%s:%s/');"
    return "ws = new WebSocket('ws://%s:%s/');"
...
function openSocket(){
    try{
    """ + getconnstring() % (net_interface_ip, wsport) +  """
...
</script>""" % (pending_messages_queue_length, websocket_timeout_timer_ms)
